    ...
    if (stompClient && stompClient.connected) {
      stompClient.publish({
        ...
        body: JSON.stringify({
          ...
          sourceColumn: selectedPiece.column,
          destinationRow: moveLocation.row,
          destinationColumn: moveLocation.column,
          gameId: game.id,
          pieceId: selectedPiece.id,
          playerId: selectedPiece.player.id,
        }),
      });
    } else {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      for (let j = 0; j < 8; j++) {
        ...
        let piece = game.pieces.find(
          (element) => element.row === i && element.column === j,
        );
        row.push(
          <div
            key={count}
            className={`${className} grid-box `}
            onClick={() => moveTo({ row: i, column: j })}
          >
            {piece ? (
              <GamePiece
                piece={piece}
